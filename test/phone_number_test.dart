...
//
// Ported from origin/dev. The expected E.164 output is pinned against the
// backend's regex ^\+234[7-9][0-1][0-9]{8}$ and against a number confirmed
// working via a live 200 from POST /api/v1/auth/send-otp.
...
void main() {
  ...
  group('rejects invalid input', () {
    ...
    test('a landline-looking number is not a mobile number', () {
      expect(normalizeNigerianPhone('01234567890'), isNull);
    });
  });
  ...
  group('nigerianPhoneError', () {
    test('returns null for a valid number', () {
      ...
      // Signup collects the 10 subscriber digits after a fixed +234.
      expect(nigerianPhoneError('7015697383'), isNull);
    });
    ...
    test('uses the PRD wording when invalid', () {
      expect(nigerianPhoneError('123'), 'Invalid Nigerian mobile number');
      // A 10-digit entry that still isn't a valid prefix — the case the old
      // "length == 10" check let through to a server 400.
      expect(nigerianPhoneError('6015697383'), 'Invalid Nigerian mobile number');
    });
  });
}
